...
#import sys
#sys.path.insert(0, "/path/to/pycocotools")
...
class RandomHorizontalFlipDict(object):
    ...
    def __call__(self, sample):
        if random.random() < self.p:
            ...
            width, _ = image.shape[:2]
            annots[:, 1] = width - annots[:, 1] - 1
            annots[:, 3] = width - annots[:, 3] - 1
...
def main(args=None):
    ...
    if parser.dataset == 'coco':
        ...
        dataset_train = CocoDataset(parser.coco_path, set_name='train2017',
                            transform=transforms.Compose([Normalizer(), Augmenter(), RandomHorizontalFlipDict(p=0.5), Resizer()]))
